...
impl Bitfield {
	...
	fn range_update<R: RangeBounds<usize>, F: Fn(&mut u8, u8)>(&mut self, range: R, update_block: F) -> &mut Self {
		...
		for index in start_index..=end_index {
			...
			update_block(&mut self.data[index], bitmask_between(index_start_bit, index_end_bit));
		}
