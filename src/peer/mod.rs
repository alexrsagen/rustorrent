...
impl Peer {
		...
		match msg {
			...
			Message::Have(index) => {
				...
				if bitfield_guard.is_none() {
					*bitfield_guard = Some(Bitfield::new(self.torrent.metainfo.info.piece_hashes.len()));
				}
				if crate::DEBUG {
			...
			Message::Request(request) => {
				if crate::DEBUG {
					println!("[debug] peer {} requested block of piece {} (offset {}, length {})", &self.info, request.index, request.begin, request.length);
				}
	...
	pub async fn keepalive(&self) -> Result<(), Error> {
		...
		let keepalive_interval = conn.opts.keepalive_interval - conn.opts.tx_timeout.unwrap_or(Duration::from_secs(5));
	...
	pub async fn run_event_loop(&self) -> Result<(), PeerError> {
		...
		loop {
			...
			match self.read().await {
				Ok(msg) => {
					match msg {
						Message::Port(dht_port) => {
							if crate::DEBUG {
								println!("[debug] peer {} sent DHT port: {:?}", &self.info, dht_port);
							}
							// TODO: DHT related, insert peer into "local routing table"
						},
						msg => {
							if crate::DEBUG {
								println!("[debug] peer {} sent unhandled message: {:?}", &self.info, msg);
							}
						},
					}
				},
				Err(Error::Timeout(_)) => {},
				Err(Error::MessageHandled) => {},
				Err(Error::UnsolicitedPieceBlockMessage { piece_index, block_index }) => {
					println!("[warning] peer {} sent unsolicited block {} of piece {}", &self.info, block_index, piece_index);
				},
				Err(Error::InvalidPieceBlockMessage { piece_index, block_index }) => {
				...
				Err(Error::PieceHashInvalid { piece_index }) => {
					println!("[warning] piece {} could not be verified and will be re-downloaded", piece_index);
				},
			...
			if let Some(msg_interest) = msg_interest {
				...
				match self.write(msg_interest.clone()).await {
					Ok(_) => {
						if crate::DEBUG {
							println!("[debug] updated peer {} interest: {:?}", &self.info, msg_interest);
						}
					},
					Err(Error::NoOp) => {},
			...
			if !self.state().await.map_err(|e| PeerError::ConnectionError(e))?.peer_choking {
			...
			self.keepalive().await.map_err(|e| PeerError::WriteError(e))?;
	...
	pub async fn connect_and_run_event_loop(self: Arc<Self>) -> Result<(), PeerError> {
		self.connect().await.map_err(|e| PeerError::ConnectionError(e))?;
		...
		match self.handshake().await {
			Ok(_) => {
				...
				if !bitfield.is_all_clear() {
					if crate::DEBUG {
						println!("[debug] sending bitfield to peer {}: {:?}", &self.info, &bitfield);
					}
					match self.write(Message::Bitfield(bitfield)).await {
						Ok(_) => {
							match self.run_event_loop().await {
								...
								Err(e) => {
									if crate::DEBUG {
										println!("[debug] disconnected from peer {}: {:?}", &self.info, e);
									}
								},
							};
						},
						Err(e) => {
							println!("[warning] unable to send our bitfield to peer {}: {:?}", &self.info, e);
						}
					}
				}
				...
				if let Some(peer_bitfield) = self.bitfield.read().await.as_ref() {
					let _ = self.torrent.pieces.decrease_availability(peer_bitfield).await;
				}
			},
			Err(e) => {
				println!("[warning] unable to handshake with peer {}: {:?}", &self.info, e);
			}
		}
