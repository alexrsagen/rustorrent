...
impl PeerInfo {
	...
	pub fn new_local(prefix: &'static str, ip: IpAddr, port_range: PortRange) -> Self {
		...
		let id_rand: String = (&mut rng).sample_iter(&rand::distributions::Alphanumeric).take(20 - prefix.len()).map(char::from).collect();
...
impl From<[u8; 18]> for PeerInfo {
	fn from(value: [u8; 18]) -> Self {
		Self::new(None, SocketAddr::new(
			IpAddr::V6(Ipv6Addr::new(
				u16::from_be_bytes([value[0], value[1]]), u16::from_be_bytes([value[2], value[3]]),
				u16::from_be_bytes([value[4], value[5]]), u16::from_be_bytes([value[6], value[7]]),
				u16::from_be_bytes([value[8], value[9]]), u16::from_be_bytes([value[10], value[11]]),
				u16::from_be_bytes([value[12], value[13]]), u16::from_be_bytes([value[14], value[15]]),
			)),
...
impl TryFrom<bencode::Dict> for PeerInfo {
	...
	fn try_from(mut dict: bencode::Dict) -> Result<Self, Self::Error> {
		...
		if let Some(bencode::Value::Bytes(v)) = dict.remove("peer id") {
			id = Some(v.try_into().map_err(|_| Error::ValueTypeMissingOrInvalid("peer id".into()))?);
		}
		let ip: IpAddr;
...
impl From<&PeerInfo> for bencode::Dict {
	fn from(value: &PeerInfo) -> Self {
		...
		dict.insert("ip".into(), bencode::Value::Bytes(value.addr.ip().to_string().as_bytes().to_vec()));
