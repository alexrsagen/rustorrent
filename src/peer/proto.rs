...
impl TryFrom<&[u8]> for Handshake {
    ...
    fn try_from(data: &[u8]) -> Result<Self, Self::Error> {
        ...
        if pstr != PSTR {
            return Err(Error::ProtoInvalid(InvalidProto::Peer(PeerProto::Other(
                pstr,
            ))));
        }
        i += pstrlen;
        ...
        i += 8;
        let info_hash = data[i..i + 20]
            .try_into()
            .map_err(|_| Error::MessageLengthInvalid)?;
        i += 20;
        let peer_id = data[i..i + 20]
            .try_into()
            .map_err(|_| Error::MessageLengthInvalid)?;
        Ok(Self {
            pstr,
...
impl From<Message> for Vec<u8> {
    fn from(value: Message) -> Self {
        ...
        match value {
            ...
            Message::Keepalive => {}
            Message::Choke => {
                bytes.push(Message::ID_CHOKE);
            }
            Message::Unchoke => {
                bytes.push(Message::ID_UNCHOKE);
            }
            Message::Interested => {
                bytes.push(Message::ID_INTERESTED);
            }
            Message::NotInterested => {
                bytes.push(Message::ID_NOTINTERESTED);
            }
            Message::Have(v) => {
