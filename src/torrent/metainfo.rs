...
impl fmt::Display for File {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match self.md5sum {
			Some(md5sum) => write!(f, "{} ({}, md5sum: {:x?})", self.path, BytesBase2::from_bytes(self.length), md5sum),
...
impl TryFrom<bencode::Dict> for Metainfo {
	...
	fn try_from(mut dict: bencode::Dict) -> Result<Self, Self::Error> {
		...
		if let Some(info_value) = dict.remove("info") {
			...
			if let bencode::Value::Dict(mut info) = info_value {
				...
				if let Some(bencode::Value::Bytes(pieces_v)) = info.remove("pieces") {
					piece_hashes = pieces_v.chunks_exact(20).map(|chunk| chunk.try_into().unwrap()).collect();
				} else {
				...
				if let Some(bencode::Value::List(file_list_v)) = info.remove("files") {
					...
					file_list = file_list_v.into_iter().filter_map(|file_v| {
						if let bencode::Value::Dict(mut file_dict) = file_v {
							let path: String;
							let length: usize;
							let mut md5sum: Option<[u8; 16]> = None;

							if let Some(bencode::Value::Bytes(path_v)) = file_dict.remove("path") {
							...
							if let Some(bencode::Value::Int(length_v)) = file_dict.remove("length") {
								length = length_v as usize;
							} else {
								return None;
							}
							if let Some(bencode::Value::Bytes(md5sum_v)) = file_dict.remove("md5sum") {
							...
							return Some(File {path, length, md5sum})
						}
						None
					}).collect();
		...
		if let Some(bencode::Value::List(tier_list)) = dict.remove("announce-list") {
			let tier_list: Vec<Vec<String>> = tier_list.into_iter().map(|announce_list_v| {
				if let bencode::Value::List(announce_list) = announce_list_v {
					return announce_list.into_iter().map(|announce_v| {
		...
		if let Some(bencode::Value::Int(creation_date_v)) = dict.remove("creation date") {
			creation_date = Some(DateTime::<Utc>::from(UNIX_EPOCH + Duration::from_secs(creation_date_v as u64)));
		}
...
impl From<&Metainfo> for bencode::Dict {
	fn from(value: &Metainfo) -> Self {
		...
		info.insert("piece length".into(), bencode::Value::Int(value.info.piece_length as i64));
		...
		match &value.info.files {
			Files::Multiple(dir) => {
				info.insert("name".into(), bencode::Value::Bytes(dir.name.as_bytes().to_vec()));
				info.insert("files".into(), bencode::Value::List(dir.files.iter().map(|file| {
					let mut file_dict = bencode::Dict::new();
					file_dict.insert("path".into(), bencode::Value::Bytes(file.path.as_bytes().to_vec()));
					file_dict.insert("length".into(), bencode::Value::Int(file.length as i64));
					if let Some(md5sum) = file.md5sum {
						file_dict.insert("md5sum".into(), bencode::Value::Bytes(md5sum.to_vec()));
					}
					bencode::Value::Dict(file_dict)
				}).collect()));
			},
			Files::Single(file) => {
				info.insert("name".into(), bencode::Value::Bytes(file.path.as_bytes().to_vec()));
		...
		dict.insert("announce".into(), bencode::Value::Bytes(value.announce[0][0].as_bytes().to_vec()));
		if value.announce.len() > 1 {
			dict.insert("announce-list".into(), bencode::Value::List(value.announce.iter().map(|tier| {
				bencode::Value::List(tier.iter().map(|announce| bencode::Value::Bytes(announce.as_bytes().to_vec())).collect())
			}).collect()));
		}

		if let Some(creation_date) = value.creation_date {
			dict.insert("creation date".into(), bencode::Value::Int(creation_date.timestamp()));
		}

		if let Some(comment) = &value.comment {
			dict.insert("comment".into(), bencode::Value::Bytes(comment.as_bytes().to_vec()));
		}

		if let Some(created_by) = &value.created_by {
			dict.insert("created by".into(), bencode::Value::Bytes(created_by.as_bytes().to_vec()));
		}

		if let Some(encoding) = &value.encoding {
			dict.insert("encoding".into(), bencode::Value::Bytes(encoding.as_bytes().to_vec()));
		}
