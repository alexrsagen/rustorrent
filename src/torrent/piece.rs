...
pub fn get_block_size(piece_size: usize, block_size: usize, block_index: usize) -> Result<usize, Error> {
...
pub fn get_block_index(piece_size: usize, block_size: usize, begin: usize, length: usize) -> Result<usize, Error> {
...
impl PieceStore {
	...
	pub fn new(piece_count: usize, piece_size: usize, block_size: usize) -> Self {
		...
		Self {
			block_size,
			pieces: (0..piece_count).map(|_| Arc::new(Piece::new(piece_size, block_size))).collect(),
	...
	pub async fn increase_availability(&self, bitfield: &Bitfield) -> Result<(), Error> {
		...
		for piece_index in 0..bitfield.len() {
			if bitfield.get(piece_index) {
				self.pieces[piece_index].availability.fetch_add(1, Ordering::SeqCst);
				modified = true;
			}
		}
	...
	pub async fn decrease_availability(&self, bitfield: &Bitfield) -> Result<(), Error> {
		...
		for piece_index in 0..bitfield.len() {
			if bitfield.get(piece_index) {
				self.pieces[piece_index].availability.fetch_sub(1, Ordering::SeqCst);
				modified = true;
			}
		}
		if modified && self.pick_mode.load() != PickMode::Random {
...
impl Piece {
	...
	pub fn new(piece_size: usize, block_size: usize) -> Self {
		Self {
			...
			blocks: Mutex::new(vec![PieceBlockState::Open; get_block_count(piece_size, block_size)]),
	...
	pub fn priority(&self) -> usize {
		let not_partial = if self.state.load() == PieceState::Partial { 0 } else { 1 };
		self.availability.load(Ordering::SeqCst) * 2 + not_partial
	}

	/// retval = a complete piece was assembled and verified
	async fn write_block(&self, peer: &Peer, block: PieceBlock, block_size: usize) -> Result<(), Error> {
		let piece_index = block.index as usize;
		if piece_index >= peer.torrent.metainfo.info.piece_hashes.len() {
			return Err(Error::InvalidPieceBlockMessage{ piece_index: 0, block_index: 0 });
		}
		...
		let block_index = get_block_index(piece_size, block_size, block_begin, block.data.len())
			.map_err(|_| Error::InvalidPieceBlockMessage{ piece_index, block_index: 0 })?;

		// get block state
		let mut blocks = self.blocks.lock().await;
		if let PieceBlockState::Requested(req) = &blocks[block_index] {
			if !peer.eq(&req.peer) {
				return Err(Error::UnsolicitedPieceBlockMessage{ piece_index, block_index });
			}
			if req.request.index != block.index {
				return Err(Error::InvalidPieceBlockMessage{ piece_index, block_index });
			}
		...
		data.buffer_mut(piece_size).splice(block_begin..block_end, block.data);

		// update block state
		blocks[block_index] = PieceBlockState::Finished;

		// check if all blocks are finished
		let all_blocks_finished = blocks.iter()
