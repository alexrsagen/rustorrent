...
use crate::error::{Error, InvalidProto, HttpProto};
...
fn tls_client(resolver: AsyncHyperResolver) -> Client<HttpsConnector<HttpConnector<AsyncHyperResolver>>> {
	...
	pub async fn request(&self, req: Request<Body>) -> Result<Response<Body>, Error> {
		match req.uri().scheme_str() {
			Some(scheme) => match scheme {
				...
				_ => Err(Error::ProtoInvalid(InvalidProto::Http(HttpProto::Other(String::from(scheme))))),
			},
...
async fn read_body_gzip(res: &mut Response<Body>) -> Result<Vec<u8>, Error> {
	let mut stream = tokio_util::codec::FramedRead::new(
		GzipDecoder::new(
			tokio_util::io::StreamReader::new(IoStream(res.body_mut()))
		),
